...
impl<'a> Emitter<'a> {
  ...
  fn emit_interface(&mut self, interface: &Interface, interior: bool) -> Wire {
    if let InterfaceKind::Inspect(_) = &interface.kind {
      todo!()
    } else {
      let vars = interface
        .wires
  ...
  fn emit_step(&mut self, step: &Step) {
    match step {
      ...
      Step::BoolNot(a, b) => {
        let a = self.emit_port(a);
        let b = self.emit_port(b);
        self.net.add(self.guide.bool_not, a, [b]);
      }
      Step::BoolAnd(a, b, c) => {
        let a = self.emit_port(a);
        let b = self.emit_port(b);
        let c = self.emit_port(c);
        self.net.add(self.guide.bool_and, a, [b, c]);
      }
    }
  }
  ...
  pub(crate) fn emit_port(&mut self, port: &Port) -> Wire {
    match port.kind {
      ...
      PortKind::Bool(b) => self.net.make(self.guide.bool.with_data(b), []),
      PortKind::N32(n) => self.net.make(self.guide.n32.with_data(n), []),
      PortKind::F32(n) => self.net.make(self.guide.f32.with_data(n), []),
      PortKind::F64(n) => self.net.make(self.guide.f64.with_data(n), []),
