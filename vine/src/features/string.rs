...
impl Emitter<'_> {
  pub(crate) fn emit_string(&mut self, port: &Port, init: &str, rest: &Vec<(Port, String)>) {
    ...
    for char in init.chars() {
      ...
      let char = self.net.make(self.guide.n32.with_data(char as u32), []);
    ...
    if static_len != 0 {
      lens.push(self.net.make(self.guide.n32.with_data(static_len), []));
    }

    let len = if lens.is_empty() {
      self.net.make(self.guide.n32, [])
    } else {
      let cur = lens.pop().unwrap();
      for len in lens {
        let new = self.net.wire();
        self.net.add(self.guide.n32_add, cur, [len, new]);
      }
      cur
    };
