...
impl<'core> Charter<'core, '_> {
      ...
      ItemKind::Enum(enum_item) => {
        ...
        let variants = enum_item
          ...
          .map(|(id, variant)| {
            ...
            Variant { def, name: variant.name, data: variant.data }
          })
          .collect::<Vec<_>>()
          .into();
  ...
  fn chart_builtin(&mut self, def_id: Option<DefId>, builtin: Builtin) -> bool {
    ...
    let struct_ = || {
      def.type_def.and_then(|id| {
        if let TypeDefKind::Struct(id) = self.chart.types[id].kind {
          Some(id)
        } else {
          None
        }
    ...
    match builtin {
      ...
      Builtin::List => set(struct_(), &mut self.chart.builtins.list),
      Builtin::String => set(struct_(), &mut self.chart.builtins.string),
