...
use ivy::text::ast::Expr as IvyExpr;
...
pub trait VisitMut<'a> {
  ...
  fn _visit_expr(&mut self, expr: &'a mut Expr) {
    match &mut *expr.kind {
      ...
      ExprKind::Fn(_, a, b, c) | ExprKind::Bench(_, a, b, c) => {
        self.visit(a);
        self.visit(b);
        self.visit_block(c);
      }
      ...
      ExprKind::Nat(_, _, t) | ExprKind::Float(_, _, t) => {
        self.visit(t);
      }
    }
  }
  ...
  fn _visit_stmt(&mut self, stmt: &'a mut Stmt) {
    match &mut stmt.kind {
      ...
      StmtKind::Let(l) => {
        if let LetStmtKind::Init(init) = &mut l.kind {
