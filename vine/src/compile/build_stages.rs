...
impl Compiler {
  ...
  fn build_expr(&mut self, term: &Term, ctx: ExprCtx) -> Expr {
    match &term.kind {
      ...
      TermKind::Fn(params, body) => {
        ...
        let old_return = self.return_target.replace((result, self.cur.fork));
      ...
      TermKind::While(cond, body) => {
        self.fork(|slf, f| {
          let start = slf.new_stage(f, move |slf, f, start| {
            let cond = slf.build_expr_value(cond);

            slf.fork(|slf, g| {
              let body = slf.new_stage(g, |slf, _, _| {
                let body = slf.build_block(body);
                slf.erase(body);
                let start = slf.stage_port(start);
                slf.cur.steps.push_back(Step::Call(f.outer, start));
                slf.diverge(f.id);
              });

              let end = slf.new_stage(g, |_, _, _| {});
              ...
              slf.cur.steps.push_back(Step::Call(g.outer, r.1));
            });
          });

          slf.cur.steps.push_back(Step::Call(f.outer, slf.stage_port(start)));
        });
      ...
      TermKind::If(cond, then, els) => {
        ...
        self.fork(|slf, f| {
          ...
          slf.cur.steps.push_back(Step::Call(f.outer, r.1));
        });
  ...
  fn new_stage(
    &mut self,
    fork: &mut Fork,
    f: impl FnOnce(&mut Self, &mut Fork, StageId),
  ) -> StageId {
    let old_id = self.cur_id;
    let id = self.start_stage(fork.id, fork.outer);
  ...
  fn start_stage(&mut self, fork: ForkId, interface: InterfaceId) -> usize {
  ...
  fn diverge(&mut self, to: ForkId) {
    ...
    self.start_stage(self.cur.fork, i);
    self.cur.divergence = divergence;
  }

  fn fork(&mut self, f: impl FnOnce(&mut Self, &mut Fork)) {
    let id = self.fork_id;
    self.fork_id += 1;
    let mut fork = Fork { id, outer: self.new_interface(), ends: Vec::new() };
    f(self, &mut fork);
    let divergence = fork.ends.iter().map(|&x| self.stages[x].divergence).min().unwrap_or(fork.id);
    if divergence < fork.id {
