...
impl FlatNet {
  ...
  pub fn to_tree(&self) -> TreeNet {
    let wire = |&wire: &Wire| TreeNode::Wire(self.links.canonicalize_ref(wire));
    TreeNet {
      wires: self.wires,
      free: self.free.iter().map(wire).collect(),
      links: self
        .nodes
        .iter()
        .map(|node| {
          (wire(&node.pri), TreeNode::Node(node.name.clone(), node.aux.iter().map(wire).collect()))
        })
        .collect(),
    }
  }
  ...
  pub fn from_tree_nets(nets: &HashMap<NameId, TreeNet>) -> HashMap<NameId, FlatNet> {
    nets.iter().map(|(&name, net)| (name, net.to_flat())).collect()
  }

  pub fn assert_valid(nets: &HashMap<NameId, FlatNet>, table: &Table) {
    let mut wires = HashMap::<Wire, usize>::new();
    let mut invalid = Vec::new();
    for (&id, net) in nets {
      let mut visit = |wire| *wires.entry(net.links.canonicalize_ref(wire)).or_default() += 1;
      for &wire in &net.free {
        visit(wire);
      }
      for node in &net.nodes {
        visit(node.pri);
        for &wire in &node.aux {
          visit(wire);
        }
      }
      let mut invalid_wires = Vec::new();
      for (wire, n) in wires.drain() {
        if n != 2 {
          invalid_wires.push(wire);
        }
      }
      if !invalid_wires.is_empty() {
        invalid.push((id, invalid_wires));
      }
    }
    if !invalid.is_empty() {
      invalid.sort_by_key(|x| x.0);

      let mut error = String::new();
      for (id, wires) in invalid {
        let (net, mut names) = Net::from_flat_with_wire_names(&nets[&id]);
        write!(
          error,
          "\ninvalid wires {} in {} {}\n",
          wires.into_iter().map(|w| names.get(w)).collect::<Vec<_>>().join(", "),
          table.show_name_id(id),
          net.print(table)
        )
        .unwrap();
      }
...
impl TreeNet {
  ...
  pub fn resolve_links(&mut self) {
        ...
        TreeNode::Wire(wire) => {
          *wire = wire_links.canonicalize(*wire);
          if let Some(extracted) = trees.remove(wire) {
            *tree = extracted;
            visit(trees, wire_links, tree);
          }
        }
        TreeNode::Node(_, children) => {
          for child in children {
            visit(trees, wire_links, child);
          }
        }
      }
    }
    ...
    if !trees.is_empty() {
      for free in &mut self.free {
        visit(&mut trees, &mut wire_links, free);
      }
      for (a, b) in &mut self.links {
        visit(&mut trees, &mut wire_links, a);
        visit(&mut trees, &mut wire_links, b);
      }
    }
  ...
  pub fn from_flat_nets(nets: &HashMap<NameId, FlatNet>) -> HashMap<NameId, TreeNet> {
    nets.iter().map(|(&name, net)| (name, net.to_tree())).collect()
  }
}
...
impl WireLinks {
  ...
  pub fn canonicalize_ref(&self, a: Wire) -> Wire {
    if let Some(&b) = self.links.get(&a) { a.min(b) } else { a }
  }
}
