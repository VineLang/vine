...
pub enum Tag {
  ...
  /// This port is a reference to a graft net. Its address is an
  /// `&'ivm`[`Graft`].
  Graft = 2,
...
impl<'ivm> Port<'ivm> {
  ...
  /// Constructs a new [`Tag::Graft`] port.
  #[inline(always)]
  pub fn new_graft(graft: &'ivm Graft) -> Self {
    unsafe { Self::new(Tag::Graft, 0, Addr(graft as *const Graft as *const ())) }
  }
  ...
  #[inline(always)]
  pub unsafe fn as_ext_fn(&self) -> ExtFnId<'ivm> {
    unsafe {
      debug_assert_eq!(self.tag(), Tag::ExtFn);
      transmute::<u16, ExtFnId>(self.label())
    }
  }
  ...
  /// This port must be [`Tag::ExtVal`].
  #[inline(always)]
  pub unsafe fn as_graft(&self) -> &'ivm Graft<'ivm> {
    unsafe {
      debug_assert_eq!(self.tag(), Tag::Graft);
      self.addr().as_graft()
    }
  }
...
impl<'ivm> Debug for Port<'ivm> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self.tag() {
      ...
      Tag::Graft => write!(f, "Graft({:?})", self.addr()),
