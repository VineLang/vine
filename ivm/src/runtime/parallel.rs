...
struct Shared<'ivm, 'ext> {
  ivm_return: OnceLock<Runtime<'ivm, 'ext>>,
  msg: Mutex<Msg<'ivm>>,
  condvar: Condvar,
}
...
struct WorkerHandle<'ivm, 'ext, 'w> {
  shared: &'w Shared<'ivm, 'ext>,
}

struct Dispatch<'ivm, 'ext, 'w> {
  active: Vec<WorkerHandle<'ivm, 'ext, 'w>>,
  idle: Vec<WorkerHandle<'ivm, 'ext, 'w>>,
}

impl<'ivm, 'ext> Dispatch<'ivm, 'ext, '_> {
  fn execute(mut self) {
    loop {
      let mut i = 0;
