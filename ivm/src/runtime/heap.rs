...
impl Heap {
  ...
  pub fn empty() -> Box<Self> {
    unsafe {
      Box::from_raw(ptr::slice_from_raw_parts_mut(ptr::dangling_mut::<Node>(), 0) as *mut Heap)
    }
  }

  #[inline(always)]
  pub(crate) fn bump<'a>(self: &mut &'a mut Self) -> Option<&'a mut Node> {
    (!self.0.is_empty()).then(|| unsafe { &mut *(self.chunk(1) as *mut Self as *mut Node) })
  }

  #[inline(always)]
  pub(crate) fn chunk<'a>(self: &mut &'a mut Self, size: usize) -> &'a mut Self {
    let (left, right) = self.0.split_at_mut(size);
    *self = unsafe { transmute::<&mut [Node], &mut Heap>(right) };
    unsafe { transmute::<&mut [Node], &mut Heap>(left) }
  }
}
