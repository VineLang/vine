...
impl<'ivm, 'ext> IVM<'ivm, 'ext> {
  ...
  pub fn normalize_breadth_first(&mut self) {
    #[cfg(not(target_arch = "wasm32"))]
    let start = Instant::now();
    let mut work_fast = vec![];
    let mut work_slow = vec![];
    while self.normalize_breadth_first_batch(&mut work_fast, &mut work_slow) > 0 {}
    #[cfg(not(target_arch = "wasm32"))]
    {
      self.stats.time_clock += start.elapsed();
    }
  ...
  pub fn normalize_breadth_first_batch(
    &mut self,
    work_fast: &mut Vec<(Port<'ivm>, Port<'ivm>)>,
    work_slow: &mut Vec<(Port<'ivm>, Port<'ivm>)>,
  ) -> usize {
    mem::swap(work_fast, &mut self.active_fast);
    mem::swap(work_slow, &mut self.active_slow);
    if work_fast.is_empty() && work_slow.is_empty() {
      return 0;
    }
    self.stats.depth += 1;
    let interactions = work_fast.len() + work_slow.len();
    for (a, b) in work_fast.drain(..).chain(work_slow.drain(..)) {
      self.interact(a, b);
    }
    interactions
  }
